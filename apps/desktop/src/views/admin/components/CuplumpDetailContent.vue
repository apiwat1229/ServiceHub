...
const lotNoSuffix = computed({
  get() {
    ...
    // Fallback: If prefix matches partially or not at all,
    // we might want to just show the full string?
    // But UI expects suffix. Let's return raw if it doesn't match standard prefix.
...
const handleUpdateLotNo = async () => {
  ...
  // The lotNoSuffix computed property handles the prefix, so we only validate the suffix part
  ...
  if (isDuplicate) {
    lotNoError.value = t('cuplump.lotNoDuplicate') || 'Lot Number already exists';
    toast.error(lotNoError.value);
    // Revert to original? Or just keep error?
    // User requested "Lot Number must not be duplicate"
    return;
  }
        ...
        <div class="space-y-2 pb-6">
          ...
          <div class="border rounded-xl overflow-hidden bg-background shadow-sm">
            <Table>
              ...
              <TableBody>
                <!-- Saved Samples (Rendered FIRST for Ascending Order) -->
                <TableRow
                ...
                >
                  <TableCell class="text-center font-medium text-xs text-muted-foreground py-1.5">{{
                    sample.sampleNo
                  }}</TableCell>
                ...
                <!-- New Samples (Rendered AFTER Saved Items) -->
                <TableRow
