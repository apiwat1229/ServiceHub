...
import { useMaintenance, type RepairPart } from '@/composables/useMaintenance';
...
const { machines, stocks } = useMaintenance();
...
<template>
  <div class="space-y-6 py-4">
    ...
    <div class="grid grid-cols-2 gap-4">
      <div class="space-y-2">
        <Label class="text-slate-700 font-semibold">{{
          t('services.maintenance.forms.repair.machine')
        }}</Label>
        <Popover v-model:open="openMachineCombo">
            ...
            >
              {{
                form.machineId
                  ...
                  : t('services.maintenance.forms.repair.selectMachine')
              }}
          ...
          <PopoverContent class="w-[300px] p-0">
            <Command>
              <CommandInput :placeholder="t('services.maintenance.forms.repair.searchMachine')" />
              <CommandList>
                <CommandEmpty>{{
                  t('services.maintenance.forms.repair.noMachineFound')
                }}</CommandEmpty>
                <CommandGroup>
                  <CommandItem
                    v-for="m in machines"
      ...
      <div class="flex gap-4">
        <div class="flex-1 space-y-2 text-slate-700">
          <Label class="font-semibold">{{ t('services.maintenance.forms.repair.date') }}</Label>
          <Popover>
            <PopoverTrigger as-child>
              <Button
                variant="outline"
    ...
    <div class="grid grid-cols-2 gap-4">
      <div class="space-y-2">
        <Label class="text-slate-700 font-semibold">{{
          t('services.maintenance.forms.repair.technician')
        }}</Label>
        <Input
          v-model="form.technician"
          :placeholder="t('services.maintenance.forms.repair.techPlaceholder')"
          class="bg-white border-slate-200"
        />
      </div>
      <div class="space-y-2">
        <Label class="text-slate-700 font-semibold"
          >{{ t('services.maintenance.forms.repair.attachment') }}
          <span class="text-xs font-normal text-slate-500"
            >({{ t('services.maintenance.forms.repair.attachmentNote') }})</span
          ></Label
        >
        <div class="flex flex-col gap-3">
          ...
          <div class="flex items-center gap-2">
            ...
            >
              ...
              {{ t('services.maintenance.forms.repair.takePhoto') }}
            </Button>
            <Button
              @click="triggerFileInput"
              variant="outline"
            ...
            >
              <Plus class="w-3.5 h-3.5" />
              {{ t('services.maintenance.forms.repair.attachFile') }}
            </Button>
          </div>
    ...
    <div class="space-y-2">
      <Label class="text-slate-700 font-semibold">{{
        t('services.maintenance.forms.repair.issue')
      }}</Label>
      <Textarea
        v-model="form.issue"
        :placeholder="t('services.maintenance.forms.repair.issuePlaceholder')"
      ...
      <div class="flex items-center justify-between">
        <div class="flex items-center gap-2">
          ...
          <h3 class="font-bold text-slate-900 uppercase text-xs tracking-wider">
            {{ t('services.maintenance.forms.repair.partsResources') }}
          </h3>
        </div>
        <Button
          size="sm"
        ...
        >
          ...
          {{ t('services.maintenance.forms.repair.addPart') }}
        </Button>
      </div>
      ...
      <div class="grid grid-cols-12 gap-3 items-end">
        <div class="col-span-12 md:col-span-5 space-y-1.5">
          <Label class="text-[10px] uppercase font-bold text-slate-400">{{
            t('services.maintenance.forms.repair.itemName')
          }}</Label>
          <Popover v-model:open="openStockCombo">
            <PopoverTrigger as-child>
              ...
              >
                {{ partInput.name || t('services.maintenance.forms.repair.selectStock') }}
                <ChevronsUpDown class="ml-2 h-4 w-4 shrink-0 opacity-50" />
              </Button>
            </PopoverTrigger>
            <PopoverContent class="w-[300px] p-0">
              <Command>
                <CommandInput :placeholder="t('services.maintenance.forms.repair.searchStock')" />
                <CommandList>
                  <CommandEmpty>{{
                    t('services.maintenance.forms.repair.noStockFound')
                  }}</CommandEmpty>
                  <CommandGroup>
                    <CommandItem
                    ...
                    >
                      ...
                      <div class="flex flex-col">
                        ...
                        <span class="text-[10px] text-muted-foreground"
                          >{{ stock.qty }} {{ stock.unit || t('services.maintenance.units') }}
                          {{ t('services.maintenance.forms.repair.remaining') }}</span
                        >
                      </div>
                    </CommandItem>
                  </CommandGroup>
        ...
        <div class="col-span-4 md:col-span-2 space-y-1.5">
          <Label class="text-[10px] uppercase font-bold text-slate-400">{{
            t('services.maintenance.forms.repair.qty')
          }}</Label>
        ...
        <div class="col-span-8 md:col-span-5 space-y-1.5">
          <Label class="text-[10px] uppercase font-bold text-slate-400">{{
            t('services.maintenance.forms.repair.unitPrice')
          }}</Label>
          ...
          <TableHeader class="bg-slate-50/50">
            <TableRow>
              <TableHead class="text-[10px] font-black uppercase text-slate-500">{{
                t('services.maintenance.forms.repair.resource')
              }}</TableHead>
              ...
              <TableHead class="text-[10px] font-black uppercase text-slate-500 text-right">{{
                t('services.maintenance.forms.repair.cost')
              }}</TableHead>
