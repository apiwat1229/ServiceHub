    ...
    <div class="p-8 border-b-4 border-slate-900 bg-white">
      <div class="flex items-start justify-between mb-4">
        <div class="space-y-1">
          ...
          >
            {{ t('services.maintenance.officialMaintenanceRecord') }}
          </p>
          ...
          >
            {{
              repair.totalCost > 10000
                ? t('services.maintenance.majorRepair')
                : t('services.maintenance.routineService')
            }}
          </Badge>
        </div>
      </div>
    ...
    <div class="p-8 flex-1 overflow-y-auto space-y-10 min-h-0 bg-white">
      ...
      <section class="grid grid-cols-2 gap-12">
        <div class="space-y-4">
          <div class="flex items-center gap-2 border-b border-slate-100 pb-2">
            ...
            <h3 class="text-[0.625rem] font-black uppercase tracking-widest text-slate-900">
              {{ t('services.maintenance.personnelInCharge') }}
            </h3>
          </div>
          <div class="space-y-1">
            <p class="text-sm font-black text-slate-900">
              {{ repair.technician || t('services.maintenance.notAssigned') }}
            </p>
            <p class="text-[0.625rem] text-slate-400 font-bold uppercase tracking-widest">
              {{ t('services.maintenance.certifiedTechnicalLead') }}
            </p>
          </div>
        </div>
        ...
        <div class="space-y-4">
          <div class="flex items-center gap-2 border-b border-slate-100 pb-2">
            ...
            <h3 class="text-[0.625rem] font-black uppercase tracking-widest text-slate-900">
              {{ t('services.maintenance.timestamp') }}
            </h3>
          </div>
          <div class="space-y-1">
            <p class="text-sm font-black text-slate-900">{{ formatDate(repair.date) }}</p>
            <p class="text-[0.625rem] text-slate-400 font-bold uppercase tracking-widest">
              {{ t('services.maintenance.recordedDateOfService') }}
            </p>
          </div>
        </div>
      </section>
      ...
      <section class="space-y-4">
        <div class="flex items-center gap-2 border-b border-slate-100 pb-2">
          ...
          <h3 class="text-[0.625rem] font-black uppercase tracking-widest text-slate-900">
            {{ t('services.maintenance.descriptionOfIssueDetail') }}
          </h3>
        </div>
      ...
      <section v-if="repair.images && repair.images.length > 0" class="space-y-4">
        <div class="flex items-center gap-2 border-b border-slate-100 pb-2">
          ...
          <h3 class="text-[0.625rem] font-black uppercase tracking-widest text-slate-900">
            {{ t('services.maintenance.evidenceAttachments') || 'EVIDENCE / ATTACHMENTS' }}
          </h3>
        </div>
      ...
      <section class="space-y-4">
        <div class="flex items-center gap-2 border-b border-slate-100 pb-2">
          ...
          <h3 class="text-[0.625rem] font-black uppercase tracking-widest text-slate-900">
            {{ t('services.maintenance.resourcesMaterialsInventory') }}
          </h3>
        </div>
        ...
        >
          <table class="w-full text-left border-collapse">
            ...
            >
              <tr>
                <th class="px-4 py-3">{{ t('services.maintenance.materialCodeDescription') }}</th>
                <th class="px-4 py-3 text-right">{{ t('services.maintenance.quantity') }}</th>
                <th class="px-4 py-3 text-right">{{ t('services.maintenance.unitPrice') }}</th>
              </tr>
            </thead>
              ...
              >
                <td class="px-4 py-4">
                  ...
                  <p class="text-[0.5625rem] font-bold text-slate-400 uppercase tracking-widest">
                    {{ part.code || t('services.maintenance.notApplicable') }}
                  </p>
                </td>
                <td class="px-4 py-4 text-right">
                  ...
                  <span class="text-[0.625rem] font-bold text-slate-400 ml-1 uppercase">{{
                    part.unit || t('services.maintenance.units')
                  }}</span>
                </td>
                <td class="px-4 py-4 text-right">
        ...
        >
          <p class="text-slate-400 text-[0.625rem] font-black uppercase tracking-widest">
            {{ t('services.maintenance.noReplacementMaterialsUtilized') }}
          </p>
        </div>
      </section>
    </div>
    ...
    >
      <div class="text-center sm:text-left">
        <p class="text-[0.5625rem] font-black uppercase tracking-[0.2em] text-slate-400 mb-1">
          {{ t('services.maintenance.totalEconomicValuation') }}
        </p>
      ...
      <div class="flex items-center gap-3">
        ...
        <Button
          ...
          @click="emit('close')"
        >
          {{ t('services.maintenance.closeRecord') }}
        </Button>
      </div>
    </div>
  </DialogContent>
