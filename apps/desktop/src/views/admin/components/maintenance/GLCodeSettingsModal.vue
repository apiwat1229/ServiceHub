...
import { useMaintenance, type GLCode } from '@/composables/useMaintenance';
...
const { t } = useI18n();
const { glCodes, addGLCode, updateGLCode, deleteGLCode, loadData } = useMaintenance();
      ...
      >
        <div>
          <DialogTitle class="text-xl font-bold flex items-center gap-2">
            {{ t('services.maintenance.glCodes.title') }}
          </DialogTitle>
          <p class="text-sm text-slate-500 mt-1">
            {{ t('services.maintenance.glCodes.subtitle') }}
          </p>
        </div>
        <Button
          v-if="!isAdding"
        ...
        >
          <Plus class="w-3 h-3" />
          {{ t('services.maintenance.glCodes.add') }}
        </Button>
      </DialogHeader>
      ...
      <div class="flex-1 overflow-y-auto p-6 pt-2">
        ...
        >
          <h3 class="font-bold text-sm text-slate-700">
            {{
              editingId ? t('services.maintenance.glCodes.edit') : t('services.maintenance.glCodes.add')
            }}
          </h3>
          <div class="grid grid-cols-12 gap-3">
            <div class="space-y-1 col-span-2">
              <Label class="text-[10px] uppercase text-slate-500 font-bold">{{
                t('services.maintenance.glCodes.transactionId')
              }}</Label>
              <Input
            ...
            <div class="space-y-1 col-span-4">
              <Label class="text-[10px] uppercase text-slate-500 font-bold">{{
                t('services.maintenance.glCodes.description')
              }}</Label>
              <Input
            ...
            <div class="space-y-1 col-span-2">
              <Label class="text-[10px] uppercase text-slate-500 font-bold">{{
                t('services.maintenance.glCodes.code')
              }}</Label>
              <Input v-model="form.code" placeholder="6000-42" class="bg-white h-9 text-xs" />
            </div>
            <div class="space-y-1 col-span-4">
              <Label class="text-[10px] uppercase text-slate-500 font-bold">{{
                t('services.maintenance.glCodes.purpose')
              }}</Label>
              <Input
          ...
          <div class="flex justify-end gap-2">
            <Button variant="ghost" size="sm" @click="cancelAdd" class="text-xs h-8">{{
              t('services.maintenance.glCodes.cancel')
            }}</Button>
            <Button
              size="sm"
              @click="handleAdd"
            ...
            >
              {{
                editingId
                  ? t('services.maintenance.glCodes.update')
                  : t('services.maintenance.glCodes.save')
              }}
            </Button>
          </div>
        </div>
